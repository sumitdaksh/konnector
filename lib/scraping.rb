module Scraping
  def self.web_site_data(parmas_data)
    begin
      ...
      number_of_elements = parmas_data[:n]
      puts "URL: #{url}  Number of elements: #{number_of_elements}"
      doc = Nokogiri::HTML(connect_driver(url, 5, number_of_elements))
      content = doc.css('._company_86jzd_338')
      content.take(number_of_elements).each do |company|
        child_url = "https://www.ycombinator.com#{company.attribute_nodes[1].text}"
        child_res = HTTParty.get(child_url)

        # child_data = Nokogiri::HTML(connect_driver(child_url))
        child_data = Nokogiri::HTML(child_res.body)
        web_site_url = child_data.css('.group').css('a').text
        founder_names = []
        child_data.css('.space-y-5').children.each do |chi|
          linkedin = chi.css('.bg-image-linkedin').empty? ? "No linkedin" : chi.css('.bg-image-linkedin')[0].attribute_nodes[0].text
          founder_names << [chi.css('h3').text, linkedin]
        end
        if founder_names.empty?
          child_data.css('.space-y-4').children.each do |chi|
            linkedin = chi.css('.bg-image-linkedin').empty? ? "No linkedin" : chi.css('.bg-image-linkedin')[0].attribute_nodes[0].text
            founder_names << [chi.css('.leading-snug').css('.font-bold').text, linkedin]
          end
        end
        data << {
      ...
      create_csv(data, number_of_elements)
    rescue => e
      puts e
    end
  end

  def self.create_csv(data,number_of_elements)
  ...
  def self.connect_driver(url, seconds=0, number_of_elements=40)
    ...
    page_elements = driver.find_elements(:class, '_company_86jzd_338').count
    check_elements(driver, page_elements, number_of_elements)
  ...
  def self.check_elements(driver, page_elements=40, number_of_elements=40, n=0)
   return true if page_elements == driver.find_elements(:class, '_company_86jzd_338').count && n > 0
   page_elements = driver.find_elements(:class, '_company_86jzd_338').count
   puts "#{page_elements} elements #{number_of_elements}"
    if page_elements <= number_of_elements
      page_scroll(driver)
      n += 1
      check_elements(driver, page_elements, number_of_elements,n)
    end
  end

  def self.page_scroll(driver, seconds=1)
    puts "Scrolling"
